                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <a class="btn btn-primary text-white" href="tambah-artikel.php">Tambah Artikel</a>
                        </div>
                        <div class="card-body">
                                <div class="card-body">
                               <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                 <?php 
                                     include '../db.php';
                                            $artikel = mysqli_query($conn, "SELECT * FROM tb_artikel ORDER BY artikel_id DESC");

                                            while ($row = mysqli_fetch_array($artikel)) {
                                    ?>
                                <tr class="kolom">
                                    <td><img src="../web/gambar/<?php echo $row['gambar_artikel'] ?>"width = "100px"></td>
                                    <td><?php echo $row['judul_artikel'] ?></td>
                                    <td><?php echo $row['tanggal'] ?></td>
                                    <?php 

                                    $hit = mysqli_query($conn, "SELECT SUM(hits) AS jumlah_total FROM tb_statistik WHERE id_halaman = $row[artikel_id] GROUP BY id_halaman");
                                    if (mysqli_num_rows($hit)>0) {
                                    $h= mysqli_fetch_object($hit);
                                    ?>
                                    <td><?= $h -> jumlah_total; }else{
                                        echo'<td>0 Kali</td>';
                                    } ?> Kali</td>
                                    <td>
                                        <a href="proses-hapus.php?ida=<?php echo $row['artikel_id'] ?>" class="btn btn-primary btn-circle shadow" ><i class="fas fa-trash"></i> </a>
                                        <a class="btn btn-primary btn-circle shadow"  href="edit/edit_profile.php?id=<?php echo $row['artikel_id'] ?>"><i class="fas fa-edit"></i></a>
                                    </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
