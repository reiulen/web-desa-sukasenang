...
<html lang="en">
  <head>
    ...
    <title>Profil <?php echo  $a-> nama_desa ?></title>
  </head>
...
<header>
  ...
  <div class="container">
    <div class="row justify-content-center">
     ...
     <h1 class="nama-berita display-4 " style="font-weight: bold;">Profil</h1>
  </div>
</header>
   ...
   <div class="container">
    <?php
     $batas   = 8;
        ...
        $no = $posisi+1;
        $sql="SELECT * FROM tb_profile ORDER BY id_profile DESC LIMIT $posisi,$batas";
...
<?php
    ...
    $query2     = mysqli_query($conn, "SELECT * FROM tb_profile");
    ...
    <div class="text-center">
        <ul class="pagination">
          <li class="page-item">
              <a class="page-link" <?php if($halaman > 1){ echo "href='profil$sebelum'"; } ?>>Previous</a>
          </li>
            <?php
            for($i=1;$i<=$jmlhalaman;$i++) {
                if ($i != $halaman) {
                    echo "<li class='page-item'><a class='page-link' href='profil$i'>$i</a></li>";
                } else {
            ...
            ?>
             <li class="page-item">
                <a  class="page-link" <?php  if($halaman < $jmlhalaman) { echo "href='profil$setelah'"; } ?>>Next</a>
              </li>
        </ul>
    </div>
   </div>
   <?php }else{
        echo'<h1 class="text-center m-5 p-5" style="background-color:white; border-radius:8px; box-shadow: 15px 15px 25px rgba(0, 0, 0, 0.1);  font-weight:bold;">Tidak Ada Profil</h1>';
    }  ?>
 </section>
